		...
		<div class="col-md-8">
			<div class="card card-outline card-success">
				<div class="card-header">
					<h3 class="card-title">Sinistre</h3>
				</div>
				<div class="card-body">
					<div class="container-fluid">
						<div class="row">
							<div class="col-md-6">
								<label for="" class="control-label border-bottom border-primary">Objet</label>
								<p class="ml-2 d-list"><b><?php echo $subject ?></b></p>
								<label for="" class="control-label border-bottom border-primary">Client</label>
								<p class="ml-2 d-list"><b><?php echo $cname ?></b></p>
							</div>
							<div class="col-md-6">
								<label for="" class="control-label border-bottom border-primary">Statut</label>
								<p class="ml-2 d-list">
									<?php if($status == 0): ?>
									...
									<span class="badge btn-outline-primary btn update_status" data-id = '<?php echo $id ?>'>Modifier le statut</span>
									<?php endif; ?>
								</p>
								<label for="" class="control-label border-bottom border-primary">Besoin d’aide de</label>
								...
								<div class="card-tools">
									...
									<span class="dropleft">
										...
										<div class="dropdown-menu" style="">
									        <a class="dropdown-item edit_comment" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Modifier</a>
									        <div class="dropdown-divider"></div>
									        <a class="dropdown-item delete_comment" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Suprimé</a>
									     </div>
									</span>	
								<?php endif; ?>
								</div>
					...
					<div class="px-2">
						<form action="" id="manage-comment">
							<div class="form-group">
								...
								<label for="" class="control-label">Nouveau commentaire</label>
								<textarea name="comment" id="" cols="30" rows="" class="fom-control summernote2"></textarea>
							</div>
							<button class="btn btn-primary btn-sm float-right">Enregistré</button>
						</form>
					</div>
				</div>
			</div>
...
<script>
	...
	$('.edit_comment').click(function(){
		uni_modal("modifier votre Commentaire","manage_comment.php?id="+$(this).attr('data-id'))
	})
	$('.update_status').click(function(){
		uni_modal("mettre à jour  votre sinistre","manage_ticket.php?id="+$(this).attr('data-id'))
	})
	$('#manage-comment').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Commentaire enregistré avec succès.',"success");
					setTimeout(function(){
						location.reload()
					},1500)
				}
